...
const Seat = ({ color, ...props }) => {
    const [meshes, center] = useModel('/seat.glb')
    ...
    return (
        <a.group {...props} onPointerOver={hover} onPointerOut={unhover} scale={scale.to((s) => [s, s, 1])}>
            {meshes.map((mesh) => (
                <mesh></mesh>
                // <mesh key={mesh.geometry.uuid} position={center} geometry={mesh.geometry} material={mesh.material} castShadow receiveShadow>
...
const Cabin = ({ color = 'white', seatColor = 'white', name, ...props }) => {
    const [meshes, center] = useModel('/cabin.glb')
    return (
        <group {...props}>
...
export default ({ data, location }) => {
    ...
    return (
        <Layout location={location} title={siteMetadata.title} style={{ height: '100vh' }}>
            <Canvas concurrent invalidateFrameloop shadowMap camera={{ position: [0, 50, 100], fov: 65, near: 2, far: 600 }} gl={{ alpha: false }}>
